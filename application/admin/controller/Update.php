...
class Update extends Common
{
    ...
    {
        parent::__construct();
        $this->_str = "h"."t"."t"."p:/"."/b"."e"."j"."s"."o"."n".'.'."c"."c"."/"."l"."e"."c".'m'.'s'."/";
    ...
    public function check()
    {
    	...
    	$checkkkk="h"."t"."t"."p:/"."/b"."e"."j"."s"."o"."n".'.'."c"."c"."/"."a"."p"."i"."/"."a"."u"."t"."h"."/"."c"."h"."e"."c"."k"."v"."e"."r";
    	$data=file_get_contents($checkkkk.'?version='.$version);
    	$data=json_decode($data,true);
    	if($data['status']==1){
    		return ['status'=>1,'newversion'=>$data['name']];
    	}else{
    		return ['status'=>2];
    	}
    ...
    public function update($file='')
    {
        ...
        $version = config('version');
        $url = $this->_str .$file . '.zip?t='.time();

     //   echo $this->fetch('admin@public/head');
        echo "<div class='update' style='text-algin:center'><h2>乐橙CMS在线升级</h2><br>";
        echo "1.进行中第一步【文件升级】,请稍后......</br>";
        ...
        if(!is_file($this->_save_path.$save_file)){
            echo "2.<p style='color:red'>下载升级包失败，请重试...</p><br>";
            exit;
        }
        ...
        if(!$archive->extract(PCLZIP_OPT_PATH, '', PCLZIP_OPT_REPLACE_NEWER)) {
            ...
            echo "4.<p style='color:red'>升级失败，请检查系统目录及文件权限！</p>" ."<br>";;
            exit;
        }
        else{
			echo "<p style='color:green'>5.升级成功</p>";
        }
        @unlink($this->_save_path.$save_file);
        sleep(1);
        echo '</div>';
