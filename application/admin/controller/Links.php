...
class Links extends Common
{
    public function index(){
        if(request()->isPost()) {
            ...
            $link=db('links')->where($map)->order('sort')->select();
            foreach ($link as $k=>$v){
    ...
    public function linkState(){
        ...
        if(db('links')->where('id='.$id)->update(['open'=>$open])!==false){
    ...
    public function add(){
        if(request()->isPost()){
            ...
            return $this->fetch('admin/links/form');
        }
    }
    //修改友情链接
    public function edit(){
        if(request()->isPost()){
            ...
            $id=input('param.id');
            $info=db('links')->where(array('id'=>$id))->find();
    ...
    public function del(){
        db('links')->where(array('id'=>input('id')))->delete();
        cache('linkLists', NULL);
