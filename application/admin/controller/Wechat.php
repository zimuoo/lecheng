...
class Wechat extends Common{
    ...
    public function config()
    {
        ...
        $call_back_url = $domain_name.'/index/wechat/index';
        $this->assign("url", $url_top);
        $this->assign("call_back_url", $call_back_url);
        $this->assign('wchat_config', $wchat_config["value"]);
        $this->assign('title', '微信公众号设置');
        return $this->fetch('admin/wechat/config');
    }
   ...
   function getInstanceWchatMenu($instance_id)
    {
        $foot_menu = db('wx_menu')->where(['instance_id'=>$instance_id,'pid'=>0])->order('sort asc')->select();
        if (!empty($foot_menu)) {
            foreach ($foot_menu as $k => $v) {
                $foot_menu[$k]['child'] = '';
                $second_menu = db('wx_menu')->where(['instance_id'=>$instance_id,'pid'=>$v['menu_id']])->order('sort asc')->select();
    ...
    {
        $weixin_media_info = db('wx_media')->find($media_id);
    ...
    public function onloadMaterial()
    {
        ...
        $list = Db::name('wx_media')->where($condition)->order('sort')->paginate(array('list_rows'=>$pageSize,'page'=>$page))
            ->toArray();
        if (! empty($list)) {
